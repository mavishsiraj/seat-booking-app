...
app.post('/user', async (req, res) => {
    try {
      ...
      if (!booking) {
        await sleep(20000); //pretending this to be payment service
        const newBooking = new book({ ...req.body});
        const finalCheck = await book.findOne({ seat_number: req.body.seat_number });
            if (!finalCheck) {
                ...
                res.status(409).send('Conflict: Seat already booked');
            }
